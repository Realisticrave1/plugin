...
public class RavenCommand implements CommandExecutor, TabCompleter {
    ...
    private boolean showRavenInfo(Player player) {
        ...
        String type = playerData.getRaven().getType();
        int level = playerData.getRaven().getLevel();
        int xp = playerData.getRaven().getXp();
        int nextLevelXP = calculateXPForLevel(level + 1) - calculateXPForLevel(level);

        player.sendMessage(ChatColor.GOLD + "=== Your Raven ===");
        player.sendMessage(ChatColor.YELLOW + "Type: " + ChatColor.WHITE + formatRavenType(type));
        ...
        Map<String, RavenType> ravenTypes = plugin.getRavenManager().getRavenTypes();
        if (ravenTypes.containsKey(type)) {
            List<String> abilities = ravenTypes.get(type).getAbilities();
    ...
    private boolean listRavenTypes(Player player) {
        ...
        for (Map.Entry<String, RavenType> entry : ravenTypes.entrySet()) {
            String typeName = entry.getKey();
            RavenType ravenType = entry.getValue();

            String name = ravenType.getName();
            String typeDescription = ravenType.getDescription();

            player.sendMessage(ChatColor.YELLOW + name + ChatColor.WHITE + ": " + typeDescription);
        }

        return true;
    }

    private String formatRavenType(String typeName) {
        Map<String, RavenType> ravenTypes = plugin.getRavenManager().getRavenTypes();

        if (ravenTypes.containsKey(typeName)) {
            return ravenTypes.get(typeName).getName();
        }
