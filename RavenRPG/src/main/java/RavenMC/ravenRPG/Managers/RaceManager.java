...
public class RaceManager {
    ...
    private void createDefaultRaces() {
        ...
        dwarfAbilities.add(new RaceAbility("hammer_time", "Hammer Time", "Strike the ground to damage and stun nearby enemies", 25, 180));
        Race dwarf = new Race("dwarf", "Dwarf", "Hardy mountain folk with natural strength and crafting aptitude.", 90, 5, dwarfAbilities);
    ...
    public void setPlayerRace(Player player, String raceName) {
        ...
        // Update ability items in player's inventory
        plugin.getRaceItemManager().setupPlayerAbilityItems(player);
    ...
    private boolean executeRaceAbility(Player player, Race race, RaceAbility ability) {
        switch (race.getId() + ":" + ability.getId()) {
            ...
            case "dwarf:hammer_time":
                return handleDwarfHammerTime(player);
    ...
    private boolean handleDwarfHammerTime(Player player) {
        // Define ability parameters
        double radius = 5.0;          // Effect radius
        double damage = 5.0;          // Base damage
        int knockback = 1;            // Knockback strength
        int stunDuration = 3;         // Stun duration in seconds

        // Get player skill level to scale the ability
        PlayerData playerData = plugin.getPlayerManager().getPlayerData(player.getUniqueId());
        int skillLevel = playerData.getSkillLevel("combat");

        // Scale damage based on skill level
        if (skillLevel > 5) {
            damage += (skillLevel - 5) * 0.5; // +0.5 damage per level above 5
        }

        // Visual and sound effects for activation
        Location playerLoc = player.getLocation();
        player.getWorld().playSound(playerLoc, Sound.BLOCK_ANVIL_LAND, 1.0f, 0.6f);
        player.getWorld().playSound(playerLoc, Sound.ENTITY_IRON_GOLEM_ATTACK, 1.5f, 0.7f);
        ...
        // Create ground crack effect (ring of particles)
        for (double angle = 0; angle < Math.PI * 2; angle += Math.PI / 16) {
        ...
        for (Entity entity : player.getNearbyEntities(radius, 2, radius)) {
            if (entity instanceof LivingEntity && entity != player) {
                ...
                if (target instanceof Player) {
                    // For players, apply slowness and mining fatigue
                    ((Player) target).addPotionEffect(new PotionEffect(
                            PotionEffectType.SLOW,
                            stunDuration * 20, // Convert to ticks
                            2
                    ));
                    ((Player) target).addPotionEffect(new PotionEffect(
                    ...
                    // For mobs, apply slowness and show stun particles
        ...
        // Feedback to player
        if (hitSomething) {
            player.sendMessage(ChatColor.GOLD + "Your mighty hammer strike stuns nearby enemies!");
        } else {
            player.sendMessage(ChatColor.GOLD + "Your hammer strikes the ground with tremendous force!");
        }

        // Cooldown is already handled by the ability system
        return true;
    }
