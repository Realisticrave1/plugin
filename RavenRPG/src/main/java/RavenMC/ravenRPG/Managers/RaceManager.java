...
public class RaceManager {
    ...
    private boolean handleDwarfMinersSight(Player player) {
        Location center = player.getLocation();
        int radius = 16; // Larger search radius
        List<Block> nearbyOres = findNearbyOres(center, radius);

        if (!nearbyOres.isEmpty()) {
            // Highlight ores with particles
            for (Block oreBlock : nearbyOres) {
                player.getWorld().spawnParticle(Particle.VILLAGER_HAPPY, oreBlock.getLocation(), 5, 0.5, 0.5, 0.5);
            }
            player.sendMessage(ChatColor.GOLD + "Your dwarven sight reveals nearby ores!");
        } else {
